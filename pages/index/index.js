var e = require("../../utils/global"), t = require("../../utils/wxcharts"), a = (require("../../utils/html2canvas.min"), 
null);
...
Page({
    ...
    onLoad: function(t) {
        ...
        wx.onAccelerometerChange(function(t) {
            ...
            title: "将腾讯天气添加到我的小程序",
            content: "点击右上角按钮的三个小点点，即可选择将腾讯天气添加到我的小程序，快来试试吧~",
            showCancel: !1,
            confirmText: "学会了"
        }), 0 === wx.getSystemInfoSync().system.indexOf("Android") && wx.showModal({
            title: "将腾讯天气添加到手机桌面",
            content: "点击右上角按钮的三个小点点，即可选择将腾讯天气添加到手机桌面，快来试试吧~",
            showCancel: !1,
            confirmText: "学会了"
        }), wx.setStorage({
            key: "firstTime",
        ...
        wx.request({
            ...
            success: function(t) {
                ...
                if (0 === i.status && i.result) {
                    ...
                    e.searchLocation = s, wx.setStorage({
                        key: "province",
                        data: s.province
                    }), wx.setStorage({
                        key: "city",
    ...
    getInternalWeather: function(t) {
        ...
        this.setData({
            ...
            success: function(t) {
                ...
                i.fillAlarmData(s.data.alarm), i.fillHoursData(s.data.forecast_1h, s.data.rise), 
                i.fillDaysData(s.data.forecast_24h), i.fillIndexData(s.data.index, s.data.limit), 
                i.fillTipsData(s.data.tips), i.textToVoice(s.data), a());
            }
        });
    },
    ...
    fillHoursData: function(e, t) {
        ...
        if (a > s && a < o ? this.setData({
            ...
            for (var n = [], r = 0, c = 0; c < 24; c++) if (e[c]) {
                ...
                l.de = "de";
                for (var u in t) {
                    var f = t[u].time, p = new Date(f.slice(0, 4) + "/" + f.slice(4, 6) + "/" + f.slice(6, 8) + " " + t[u].sunrise).getTime(), w = new Date(f.slice(0, 4) + "/" + f.slice(4, 6) + "/" + f.slice(6, 8) + " " + t[u].sunset).getTime();
                    h > p && h - 36e5 < p && n.push({
                        time: t[u].sunrise,
                        ...
                        degree: "日落",
                        de: ""
                    }), h > w && h < p - 0 + 864e5 && ("00" !== l.weather_code && "01" !== l.weather_code && "03" !== l.weather_code && "13" !== l.weather_code || (l.weather_code = l.weather_code + "_n"));
                }
                n.push(l);
            }
            this.setData({
    ...
    textToVoice: function() {
        ...
        var t = this.data, a = new Date(), i = a.getFullYear() + "/" + (a.getMonth() + 1) + "/" + a.getDate(), s = e(a), o = e(i + " 05:00"), n = e(i + " 08:00"), r = e(i + " 11:00"), c = e(i + " 13:00"), l = e(i + " 18:00"), d = "", h = "您好";
        s > o && s <= n && (h = "早晨好"), s > n && s <= r && (h = "上午好"), s > r && s <= c && (h = "中午好"), 
        s > c && s <= l && (h = "下午好"), s > l && (h = "晚上好");
        var u = "，腾讯天气为您播报。", f = "", p = t.location;
        if ("internal" === t.addressType && (f = p.province && p.city && p.district ? p.city === p.district ? p.district : p.city + "," + p.district : p.province === p.city ? p.city : p.province + "," + p.city), 
        "tourist" === t.addressType && (f = p.tourist), "external" === t.addressType && (f = p.country + p.city), 
        s > l) {
            var w = t.daysInfo[2];
            d = h + u + f + ",明天白天到夜间," + (w.day_weather_short === w.night_weather_short ? w.day_weather_short : w.day_weather_short + "转" + w.night_weather_short) + ",温度," + (w.min_degree < 0 ? "零下" + Math.abs(w.min_degree) : w.min_degree) + "到" + (w.max_degree < 0 ? "零下" + Math.abs(w.max_degree) : w.max_degree) + "摄氏度," + (w.day_wind_power > 0 ? w.day_wind_direction + w.day_wind_power + "级。" : "") + "小贴士提醒您: " + t.weatherTips[t.weatherTipsIndex];
        } else {
            var g = t.daysInfo[1];
            d = h + u + f + ",今天白天到夜间," + (g.day_weather_short === g.night_weather_short ? g.day_weather_short : g.day_weather_short + "转" + g.night_weather_short) + ",温度," + (g.min_degree < 0 ? "零下" + Math.abs(g.min_degree) : g.min_degree) + "到" + (g.max_degree < 0 ? "零下" + Math.abs(g.max_degree) : g.max_degree) + "摄氏度," + (g.day_wind_power > 0 ? g.day_wind_direction + g.day_wind_power + "级。" : "") + "小贴士提醒您: " + t.weatherTips[t.weatherTipsIndex];
        }
        this.broadcastWeather(d);
    },
